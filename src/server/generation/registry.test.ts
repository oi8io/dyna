...
const phase = (message: string): GenerationEvent => ({
  type: "phase",
  phase: "writing",
  message,
});
...
describe("generation registry", () => {
  ...
  it("replays everything a late subscriber missed", () => {
    ...
    expect(replayed.map((event) => (event as { message: string }).message)).toEqual(
      ["a", "b"],
    );
  });
  ...
  it("replays only what comes after the last event the client saw", () => {
    ...
    expect(resumed.map((event) => (event as { message: string }).message)).toEqual(
      ["c"],
    );
  });
  ...
  it("keeps sequence numbers honest after trimming old history", () => {
    const { emit } = startRun("job-1");
    for (let i = 0; i < 4200; i += 1) emit(phase(String(i)));
    ...
    expect((all.at(-1) as { message: string }).message).toBe("4199");
  });
